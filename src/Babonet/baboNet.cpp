...
//quelques proprietes globale
	...
	uint32_t		LastClientID=0;		// keep last emitted client ID
...
cClient *getClientByID(uint32_t clientID)
{
...
int	bb_serverSetClientRate(int nbBytes,uint32_t baboNetID)
{
	if(!Server) return 1;
...
uint32_t bb_serverGetBytesSent()
{
	if(Server)
	{
...
uint32_t bb_serverGetBytesReceived()
{
	if(Server)
	{
...
int32_t bb_serverUpdate(float elapsed,int updateMsg,char* newIP)
{
	...
	switch(updateMsg)
	{
		case UPDATE_SEND_RECV: //on update les Send ET les Receive
		{
			...
			//recevoir TCP/UDP
			int32_t r = Server->ReceivePacketsFromClients();
			if(r) return r;
...
int bb_serverSend(char* dataToSend,int dataSize,int typeID,int32_t destination,int protocol)
{
	if(!Server)
	{
...
char* bb_serverReceive(uint32_t & fromID,int & typeID, int * size)
{
...
int	bb_clientSetRate(uint32_t clientID,int nbBytes)
{
...
uint32_t bb_clientConnect(const char* HostIP,unsigned short Port)
{
...
int bb_clientUpdate(uint32_t clientID,float elapsed,int updateMsg)
{
	cClient *c = getClientByID(clientID);
...
uint32_t bb_clientGetBytesSent(uint32_t clientID)
{
	cClient *c = getClientByID(clientID);
...
uint32_t bb_clientGetBytesReceived(uint32_t clientID)
{
	cClient *c = getClientByID(clientID);
...
int	bb_clientSend(uint32_t clientID,char* dataToSend,int dataSize,int typeID,int protocol)
{
	cClient *c = getClientByID(clientID);
...
char* bb_clientReceive(uint32_t clientID,int *typeID)
{
...
char* bb_clientGetLastError(uint32_t clientID)
{
...
char* bb_clientGetLastMessage(uint32_t clientID)
{
	cClient *c = getClientByID(clientID);
...
int bb_clientDisconnect(uint32_t clientID)
{
...
int bb_serverDisconnectClient(uint32_t ID)
{
	if(!Server) return 1;
...
int	bb_peerSend(char *IP,unsigned short port,char *data,int typeID,int dataSize,bool safe)
{
	...
	if(!(IP[0] >= 48 && IP[0] <= 57))
	{
		...
		char newData[sizeof(uint32_t)];
		uint32_t newid = newID;

		memcpy(newData,&newid,sizeof(uint32_t));


		if(P2P->SendTo(newID,new cUDPpacket(0,0,(char*)newData,TYPE_DEMAND,sizeof(uint32_t),true)))
		{
...
int	bb_peerSend(int32_t peerID,char *data,int typeID,int dataSize,bool safe)
{
	if(!P2P)
	{
		P2P = new cPeer2Peer();
...
char* bb_peerReceive(int32_t *fromPeerID,int *typeID)
{
...
int	bb_peerDelete(uint32_t baboNetID,bool instant)
{
...
int bb_peerGetIPport(uint32_t babonetID,char *IP,unsigned short *Port)
{
	if(!P2P) return 1;
...
uint32_t bb_peerGetBytesReceived()
{
	if(P2P)
	{
