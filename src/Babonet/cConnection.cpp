...
cConnection::cConnection(bool *udpenabled,timeval *timeout,sockaddr_in *remoteIP,int *filedescriptor,int *udpfd,char* lastError,fd_set *master,fd_set *read,fd_set *write,int *fd_max, uint32_t *lpid)
{
			...
			*Read	=	*Master;
			...
			if(FD_ISSET(*FileDescriptor,Read))
			{
				...
				if(!ToRecv) //on a recu tout ce quon attendais
				{
					...
					memcpy(Lpid , &(junk[28]) , sizeof(uint32_t) );
		...
		case 3:  //on send un udp packet au serveur
		{
			*Write = *Master;
			...
			if(FD_ISSET(*UDPfd,Write))
			{
				...
				memcpy(buf + nwrite, &connID, sizeof(uint32_t));
