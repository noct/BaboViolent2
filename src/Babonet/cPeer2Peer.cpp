...
int cPeer2Peer::SendTo(int32_t peerID,cUDPpacket *newPacket,uint32_t exception)
{
	...
	if(peerID)	//si on c fait specifier un peer
	{
		...
		for(cPeer *P=PeerList;P;P=P->Next)
		{
			if(P->ID==(uint32_t)peerID) //on a trouver notre peer
			{
...
void cPeer2Peer::ReceiveDatagram(cUDPpacket *packet,sockaddr_in fromIP)
{
	...
	if(packet->InterfaceID == 0 && packet->TypeID==TYPE_CONFIRM)
	{
		...
		uint32_t id;
		memcpy(&id,packet->Data,sizeof(uint32_t));
...
cUDPpacket* cPeer2Peer::ExtractPacket(char* buf,int *nread)
{
	...
	memcpy(&(packet->ID),buf + (*nread),sizeof(uint32_t));
...
cPeer* cPeer2Peer::GetPeerByID(uint32_t id)
{
	for(cPeer *P=PeerList;P;P=P->Next)
	{
		if(P->ID == id) return P;
...
void cPeer2Peer::AnalyzeSystemDatagram(cUDPpacket *packet,cPeer *fromPeer)
{
	...
	switch(packet->TypeID)
	{
		...
		case TYPE_DEMAND:
		{
			...
			SendTo(fromPeer->ID,new cUDPpacket(0,0,packet->Data,TYPE_CONFIRM,sizeof(uint32_t),true));
...
void cPeer2Peer::ApplyAcks(char *ackBuffer,int bufferSize,cPeer* fromPeer)
{
	...
	uint32_t *acks = new uint32_t[bufferSize/4];
...
int cPeer2Peer::AddDNSquery(char *domain,uint32_t id)
{
...
int	cPeer2Peer::DeletePeer(uint32_t peerID)
{
	for(cPeer *P = PeerList;P;P=P->Next)
	{
