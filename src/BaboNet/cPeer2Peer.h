...
public:
	...
	uint32_t	LastPeerID;			//id unique qu'on va emmetre au peers
	...
	int				AddDNSquery(char *domain,uint32_t id);											//add a dns query to the list, returns the ID giving to this queried peer
	...
	cPeer*			GetPeerByID(uint32_t id);									//retourne la reference a un peer selon son ID
	...
	int				DeletePeer(uint32_t peerID);									//permet de retirer un peer dla liste
	int				SendTo(int32_t peerID,cUDPpacket *newPacket,uint32_t exception=0);//envoie d'un packet a un peer, sauf celui quia le ID comme exception
