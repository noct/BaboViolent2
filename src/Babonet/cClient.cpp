...
cClient::cClient(sockaddr_in *ip, int fileDescriptor, uint32_t netID)
{
	UDPfd				=	0;
	UDPport				=	0;
...
cClient::cClient(const char *HostIP, unsigned short port, uint32_t netID)
{
...
int32_t cClient::IsReadyToReceive()
{
	...
	while(nread<nbytes)
	{
		...
		if(WaitingForKey)
		{
			...
			else
			{
				...
				memcpy(key, lastKey, sizeof(uint32_t));
				key[4] = '\0';
				if(stricmp("RND1",key)) return 1;	//RndLabs key is corrupted, potential hacker

				memcpy(pid, lastKey + sizeof(uint32_t), sizeof(uint32_t));
...
int cClient::Send(uint32_t &nbByte)
{
...
int cClient::SendUDP(int UDPFD,uint32_t &nbByte)
{
