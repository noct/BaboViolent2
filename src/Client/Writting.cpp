...
void Writting::updateWritting(float delay)
{
    ...
    if (dkiGetState(KeyLeft) == DKI_DOWN)
    {
        dksPlaySound(m_sfxWrite, -1, 200);
        m_cursorAnim = 0;
        m_cursorPos--;
    ...
    if (dkiGetState(KeyRight) == DKI_DOWN)
    {
        dksPlaySound(m_sfxWrite, -1, 200);
        m_cursorAnim = 0;
        m_cursorPos++;
    ...
    if (dkiGetState(KeyHome) == DKI_DOWN)
    {
        dksPlaySound(m_sfxWrite, -1, 200);
        m_cursorAnim = 0;
        m_cursorPos = 0;
    }
    if (dkiGetState(KeyEnd) == DKI_DOWN)
    {
        dksPlaySound(m_sfxWrite, -1, 200);
        m_cursorAnim = 0;
        m_cursorPos = len();
    }
    if (dkiGetState(KeyDelete) == DKI_DOWN)
    {
        dksPlaySound(m_sfxWrite, -1, 200);
        m_cursorAnim = 0;
        if (m_cursorPos < len())
