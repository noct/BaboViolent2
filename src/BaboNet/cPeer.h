...
public:
	...
	uint32_t	AcksToSend[100];		//array des acks a envoyer

	uint32_t	LastPacketID;			//garde le dernier ID de packet
	uint32_t	PendingID;				//le ID du prochain packet attendu
	...
	cPeer();
	cPeer(uint32_t ID);
	cPeer(struct sockaddr_in ip,uint32_t ID);
	...
	int				SendPackets(int fd,float elapsed,fd_set master,uint32_t & nbBytes);		//return 0 on success, 1 on failure
	...
	void			AddAck(uint32_t id);							//ajoute un ack a envoyer
