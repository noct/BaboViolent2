...
cIncConnection::cIncConnection(uint32_t id,unsigned char *netBitField,bool udpenabled,int tcpListener,int *nbClient,int *maxClient, char *lastError)
{
	ID				=	id;
			...
			switch(State)
			{
				case 0: //on doit envoyer le id dla new connection au client
				{
					if(FD_ISSET(NewFD,&fdwrite))
					{
						...
						memcpy(buf + sizeof(uint32_t),&udp,sizeof(char));
						memcpy(buf + sizeof(uint32_t) + sizeof(char),&junk,sizeof(char) * 32);

						//random starting packetID
						srand((unsigned int)(time(0)));
						Pid = rand() % 32000;
						memcpy(buf + sizeof(uint32_t) + sizeof(char) + sizeof(char) * 28,&Pid,sizeof(uint32_t));

						memcpy( &Pid , &(buf[33]) , sizeof(uint32_t) );
