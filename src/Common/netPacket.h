...
{
    char playerID; // Le ID du joueur concernê
//  char bidon[31];
};

// Le client est pret ê spawner (apparaitre)
// Il va envoyer une request au server, et ce dernier va dêcider Oê il spawn
...
// Le client tire du fusil (activitê commune chez les cocassien)
// Le server et les clients connaissent le fusil utilisê par le joueur, donc pas besoin de l'envoyer
...
struct net_clsv_player_shoot
{
    ...
    char nuzzleID; // le ID du nuzzle du fusil qui l'a tirê
    short p1[3]; // Le point du dêbut du ray
    short p2[3]; // Le point de la fin du ray
};

// La version du server est acceptê par le client
...
// Le server accept une new connection, il envoit ê tout le monde le ID du joueur
#define NET_SVCL_NEWPLAYER 101
struct net_svcl_newplayer
{
    char newPlayerID; // Le ID du nouveau Joueur (de 0 ê 31)
    int32_t baboNetID;
};

// Le server envoit ses info aux nouveaux clients
// Suivi de êa, il va lui envoyer la liste de tout les joueurs
// ê partir de ce moment lê, le joueur devra recevoir tout les messages du server
// Pour garder l'êtat du server ê jour
...
// Le server fou le camp, il le dit ê tout le monde (en moins quil plante lê)
#define NET_SVCL_SERVER_DISCONNECT 103
/*struct net_svcl_server_disconnect
{
    // Pas grand truc ê mettre ici ! Me semble que le message est clair ;)
};*/

// Le client fou le camp, le server le sait tout suite, et le shoot au autres
// (en moins quil plante lê)
...
{
    char playerID; // Le ID du joueur
    char playerName[31+1]; // Le nom du joueur, 31 + \0 caractêres
...
// Le server envoit un ping ê toute les seconde ê tout les joueurs
#define NET_SVCL_PING 106
struct net_svcl_ping
{
    char playerID; // Le ID du joueur concervê
//  char bidon[31];
};

// Le server envoit ê chaque seconde le ping de toute le monde ê tout le monde
...
// Le server reêois la request de spawner du joueur, et renvoit ê TOUT le monde
...
struct net_svcl_player_spawn
{
    ...
    short position[3]; // La position oê il spawn
...
// Le server modifie une variable sv_, il va l'envoyer ê tout le monde
// Les sv ce sont les seuls qui affectent le gameplay et que le server garde l'exclusivitêe
...
// Un client a tirê, le server a effectuê la collision et renvoi le rêsultat aux autres joueurs
#define NET_SVCL_PLAYER_SHOOT 110
struct net_svcl_player_shoot
{
    char playerID; // Le ID du joueur qui l'a tirê
    char hitPlayerID; // Le ID du joueur qu'on a touchê, si -1 on a touchê un mur
    char nuzzleID; // le ID du nuzzle du fusil qui l'a tirê (pour savoir oê spawner le feu)
    char weaponID; // Le ID du type de gun
    short p1[3]; // Le point du dêbut du ray
...
// La position d'un projectile (uniquement controlê par le server)
...
struct net_svcl_projectile_coord_frame
{
    ...
    short projectileID; // Le ID du projectile concernê
    int32_t frameID; // Le frame auquel êa a êtê envoyê (on en a de besoin pour créer er des belles interpolations)
    short position[3]; // Sa position
    char vel[3]; // Sa velocity
//  int32_t uniqueProjectileID;
    // Sa rotation sur l'axe est calculê cêtê client, vu que c uniquement visuel
};
...
struct net_svcl_explosion
{
    ...
    float radius; // Sa puissance !! (êa va aussi faire shaker la vue :P)
    char playerID;
    // On ne dit pas qui l'a provoquê et tout, c'est le server qui va faire les hits
};

// Si on a touchê un joueur l'hors d'une explosion par exemple
#define NET_SVCL_PLAYER_HIT 114
struct net_svcl_player_hit
{
    char playerID; // Le joueur touchê
    char fromID; // De qui êa vient
    char weaponID; // Le type d'arme utilisê
    float damage; // ne pas oublier le damage infligê ! ** New, la vie restante **
...
// Pour Changer l'êtat d'un flag
#define NET_SVCL_CHANGE_FLAG_STATE 118
struct net_svcl_change_flag_state
{
    char flagID; // 0 ou 1
    char newFlagState; // Son nouvel êtat
    char playerID; // Le ID du player qui a effectuê l'action
};

// Un joueur est mort ou disconnectê, il laisse tomber le flag
...
{
    char newState;
    char reInit; // Pour restarter le round ê neuf ou en parti (trace de sang, vie, etc)
};
...
// Un joueur ramasse un item, on le dit ê tout le monde
...
struct net_svcl_flame_stick_to_player
{
    ...
    char playerID; // Le ID du joueur sur qui êa stick
};
...
// Le server shoot le rêsultat des votes
...
struct net_svcl_msg
{
    ...
    char message[49+80+1]; // Null terminated string. De 79 + \0 caractêres
};
...
// Le client êcris un message, l'envoit au server
...
struct net_clsv_svcl_player_coord_frame
{
    char playerID; // Le ID du joueur concernê
    int32_t frameID; // Le frame auquel êa a êtê envoyê (on en a de besoin pour créer er des belles interpolations)
//  float angle; // Par oê il regarde
    short position[3]; // Sa position
    char vel[3]; // Sa velocity
    short mousePos[3]; // La position oê il vise
    int32_t babonetID;
    int camPosZ;
    // Son orientation sera calculê client side, vu que c pas full important c une boule
};

// On change le nom du joueur pendant le round (devra attendre la fin, ou au prochain round)
// Techniquement êa ne devrait pas être allowê, mais êa va être hot
...
// Le player shoot un projectile, on demande au server de créer er l'instance
#define NET_CLSV_SVCL_PLAYER_PROJECTILE 206
struct net_clsv_svcl_player_projectile
{
    char playerID; // Le ID du joueur
    char weaponID; // Le ID du type de gun qui a shootê le projectile
    char nuzzleID; // Le ID du nuzzle du fusil qui a l'a tirê
    char projectileType; // Le type du projectile
    short position[3]; // La position initial du projectile
    char vel[3]; // La velocitêe initial du projectile
    int32_t uniqueID;
//  int32_t uniqueProjectileID;
};
