...
public:
    ...
    uint32_t	PendingID;			// we keep the ID of the incoming packet
    uint32_t	LastPacketID;		// last emited packet ID
    ...
    uint32_t	NetID;			// un id unique du client
	...
	cClient();
	cClient(sockaddr_in *ip, int fileDescriptor, uint32_t netID);			//pour un serveur qui recois un client
	cClient(const char *HostIP, unsigned short port, uint32_t clientID);	//pour un client qui veut se connecter au serveur
	~cClient();
    ...
    int32_t			IsReadyToSend();								//to know if the server can send data to this client, returns BBNET_ERROR in case of an error, 1 in case of a 'yes', and 0 in case of a 'No'
    int32_t			IsReadyToReceive();								//to know if the server can receive data from this client, returns BBNET_ERROR in case of an error, 1 in case of a 'yes', and 0 in case of a 'No'
	...
	int				Send(uint32_t &nbByte);					//va envoyer les packets dans la liste PacketsToSend, retourn 1 si un packet n'a pas pu etre envoyer en 5 essaies, TCP
	int				SendUDP(int UDPFD, uint32_t &nbByte);		//envoie les packet UDP dans la liste de UDPPacketsToSend
