...
static void SceneRender_Intro(float t, unsigned int tex)
{
    dkglPushOrtho(1, 1);
...
static void SceneRender_Console(ClientConsole* console)
{
    int i;
    if(console->m_vPos > 0)
    {
...
static void NuzzleFlash_Render(NuzzleFlash* nf)
{
    if(nf->delay > 0)
    {
...
static void SceneRender_Weapon(ClientWeapon* weapon)
{
...
static void SceneRender_ClientPlayer(ClientPlayer* player)
{
    ...
    if(player->status == PLAYER_STATUS_ALIVE)
    {
        ...
        if(cweapon)
        {
            SceneRender_Weapon(cweapon);
        }
        if(cmeleeWeapon)
        {
            SceneRender_Weapon(cmeleeWeapon);
        }
        glRotatef(-90, 0, 0, 1);
...
static void SceneRender_ClientPlayerName(ClientPlayer* player)
{
    auto game = player->game;
    auto cgame = static_cast<ClientGame*>(player->game);
...
static void SceneRender_ClientProjectile(ClientProjectile* proj)
{
...
static void SceneRender_ClientProjectileShadow(ClientProjectile* proj)
{
    // On render son shadow :)
    glPushMatrix();
...
static void SceneRender_GameMiniMap(ClientGame* game)
{
    auto cscene = static_cast<ClientScene*>(scene);
    auto cmap = static_cast<ClientMap*>(game->map);
...
static void SceneRender_Douille(Douille* douille)
{
    glPushMatrix();
...
static void SceneRender_NukeFlash(NukeFlash* nuke)
{
    glPushMatrix();
...
static void SceneRender_Drip(Drip* drip)
{
    glPushMatrix();
...
static void SceneRender_FloorMark(FloorMark* mark)
{
...
static void SceneRender_Trail(Trail* trail)
{
...
static void SceneRender_BulletTrail(Trail* trail)
{
...
static void SceneRender_ClientGame(ClientGame* game)
{
    ...
    if(cmap)
    {
            ...
            if(cmap->weather == WEATHER_RAIN || cmap->theme == THEME_SNOW)
            {
...

                {
                    ...
                    for(i = 0; i < MAX_PLAYER; ++i)
                    {
                        ...
                        if(cplayer)
                        {
                            SceneRender_ClientPlayer(cplayer);
                        }
                    }
                    ...
                    {
                        SceneRender_ClientProjectile(static_cast<ClientProjectile*>(p));
                    }
                ...
                for(i = 0; i < (int)game->trails.size(); ++i)
                {
                    ...
                    SceneRender_Trail(game->trails[i]);
                }
                ...
                for(i = 0; i < (int)game->trails.size(); ++i)
                {
                    ...
                    glBlendFunc(GL_SRC_ALPHA, GL_ONE);
                    glBindTexture(GL_TEXTURE_2D, clientVar.tex_shotGlow);
                    SceneRender_BulletTrail(game->trails[i]);
                }
                glDepthMask(GL_TRUE);
                glPopAttrib();
        ...
        if(renderToggle >= 5)
            ...
            if(gameVar.r_showGroundMark)
            {
                for(int i = 0; i < MAX_FLOOR_MARK; ++i)
                {
                    ...
                    {
                        SceneRender_FloorMark(&game->floorMarks[i]);
                    }
                }
            }
        ...
        if(gameVar.r_projectileShadow)
        {
            ...
            for(auto p : game->projectiles)
            {
                SceneRender_ClientProjectileShadow(static_cast<ClientProjectile*>(p));
            }
        }
        ...
        {
            SceneRender_NukeFlash(game->nikeFlashes[i]);
        }
        glPopAttrib();
        ...
        for(i = 0; i < MAX_PLAYER; ++i)
        {
            ...
            {
                SceneRender_ClientPlayerName(cplayer);
            }
        }
...
static void SceneRender_StatsSlice(const CVector4f & sliceColor, char * text1, char* c1, char* c2, char* c3, char* c4, char* c5, char * pingText, int & vPos)
{
    glDisable(GL_TEXTURE_2D);
...
static void SceneRender_GameFFA(ClientGame* game, std::vector<Player*> & teamOrder, int & vPos)
{

    // All Team
    SceneRender_StatsSlice(CVector4f(1, 1, 1, .75f), CString(clientVar.lang_freeForAllC.s, game->redWin + game->blueWin).s, "", "", "", "", CString(""/*%i", blueScore + redScore*/).s, CString("%i", game->ffaPing * 33).s, vPos);
    for(int j = 0; j < (int)teamOrder.size(); ++j)
    {
        ...
        SceneRender_StatsSlice(CVector4f(0, 0, 0, .75f), showName.s, CString("%i", (int)teamOrder[j]->score).s, CString("%i", (int)teamOrder[j]->deaths).s, CString("%.1f", teamOrder[j]->dmg).s, "", "", pingStr.s, vPos);
...
static void SceneRender_GameSpectator(ClientGame* game, std::vector<Player*> & teamOrder, int & vPos)
{

    // Spectators
    SceneRender_StatsSlice(CVector4f(.5f, .5f, .5f, .75f), clientVar.lang_spectatorC.s, "", "", "", "", "", CString(""/*%i", spectatorPing*33*/).s, vPos);
    for(int j = 0; j < (int)teamOrder.size(); ++j)
    {
        CString showName = teamOrder[j]->name;
        showName.insert("\x8", 0);
        ...
        SceneRender_StatsSlice(CVector4f(0, 0, 0, .75f), showName.s,
...
static void SceneRender_GameBlueTeam(ClientGame* game, std::vector<Player*> & teamOrder, int & vPos)
{
    // Blue Team
    SceneRender_StatsSlice(CVector4f(0, 0, 1, .75f), clientVar.lang_blueTeamC.s, "", "", "", "", CString("%i", game->blueScore).s, CString(""/*%i", bluePing*33*/).s, vPos);
    for(int j = 0; j < (int)teamOrder.size(); ++j)
    {
        CString showName = teamOrder[j]->name;
...
static void SceneRender_GameRedTeam(ClientGame* game, std::vector<Player*> & teamOrder, int & vPos)
{
    // Red Team
    SceneRender_StatsSlice(CVector4f(1, 0, 0, .75f), clientVar.lang_redTeamC.s, "", "", "", "", CString("%i", game->redScore).s, CString(""/*%i", redPing*33*/).s, vPos);
    for(int j = 0; j < (int)teamOrder.size(); ++j)
    {
        CString showName = teamOrder[j]->name;
...
static void SceneRender_ClientGameStats(ClientGame* game)
{
    ...
    {
    case GAME_TYPE_DM:
        SceneRender_StatsSlice(CVector4f(0, 0, 0, .75f), clientVar.lang_playerNameC.s, "Kills", "Death", "Damage", "", "", clientVar.lang_pingC.s, vPos);
        vPos += 10;
        SceneRender_GameFFA(game, ffaTeam, vPos);
        SceneRender_GameSpectator(game, spectatorTeam, vPos);
        break;
    case GAME_TYPE_TDM:
        SceneRender_StatsSlice(CVector4f(0, 0, 0, .75f), clientVar.lang_playerNameC.s, "Kills", "Death", "Damage", "", "", clientVar.lang_pingC.s, vPos);
        vPos += 10;
        ...
        {
            SceneRender_GameBlueTeam(game, blueTeam, vPos);
            SceneRender_GameRedTeam(game, redTeam, vPos);
        }
        else
        {
            SceneRender_GameRedTeam(game, redTeam, vPos);
        ...
        break;
    case GAME_TYPE_CTF:
        SceneRender_StatsSlice(CVector4f(0, 0, 0, .75f), clientVar.lang_playerNameC.s, "Kills", "Death", "Damage", "Retrn", "Caps", clientVar.lang_pingC.s, vPos);
        vPos += 10;
...
static void SceneRender_Client(Client* client, float & alphaScope)
{
    ...
    if(client->isConnected) SceneRender_ClientGame(game);
    ...
    if(game->thisPlayer)
    {
        ...
        if(game->showStats)
        {
            SceneRender_ClientGameStats(game);
...
void ClientScene_Render(ClientScene* scene)
{
    ...
    {
        SceneRender_Intro(scene->introDelay, scene->tex_rndLogo);
    }
    else
    {
        // On render le client
        float alphaScope = 0;
        if(scene->client) SceneRender_Client(scene->client, alphaScope);
