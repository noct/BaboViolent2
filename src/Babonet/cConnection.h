...
public:
	...
	uint32_t	connID;			//garde le ID de la remote connection
    uint32_t		*Lpid;			//LastPAcketID
	...
	cConnection(bool *UDPenabled,timeval *Timeout,sockaddr_in *remoteIP,int *filedescriptor,int *udpFD,char *lastError,fd_set *master,fd_set *read,fd_set *write,int *fdmax, uint32_t *LastPacketID);
