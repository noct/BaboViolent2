...
cPeer::cPeer()
{
	...
	memset(AcksToSend,0,100*sizeof(uint32_t));
	nbAcks	=	0;
...
cPeer::cPeer(struct sockaddr_in ip,uint32_t id)
{
	Next		=	0;
	Previous	=	0;
...
int cPeer::SendPackets(int fd,float elapsed,fd_set master, uint32_t & nbBytes)
{
    ...
    if(FD_ISSET(fd,&fdwrite))
	{
		...
		if(nbAcks)
		{
			...
			ackPacket.Size			=	(unsigned short)(nbAcks * sizeof(uint32_t));
			...
			nbBytes += (nbAcks * sizeof(uint32_t));
...
int cPeer::PackToBuffer(char *buf,cUDPpacket *p)
{
	...
	//on pack le ID
	memcpy(buf + sent,&(p->ID),sizeof(uint32_t));
