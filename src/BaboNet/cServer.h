...
private:
	...
	uint32_t		LastNetID;			// garde le dernier ID emis a un client
	unsigned short		ListenPort;			// le port sur lequel on ecoute pour les new connections

	uint32_t		NewConnID;			// un ID qu'on emet a une nouvelle connection

	bool			UDPenabled;			// on est a tu UDP pour le server ?

public:

	uint32_t	BytesSent;			// nombre total de byte que le serveur a envoyer
	uint32_t	BytesReceived;		// nombre total de byte recu par le serveur
	...
	int		Send(char* dataToSend,int dataSize,int typeID,int32_t destination,int protocol=0);		//pour envoyer du server vers un client, protocol 0 = TCP(safe), 1 = UDP(unsafe)


	int32_t		ReceivePacketsFromClients();						//recevoir les packet qui viennent des clients
	...
	int32_t		CreateClient(sockaddr_in *ip, int fileDescriptor,unsigned short udpPort,uint32_t pid);	//permet d'ajouter un client
	...
	cClient*	getClientByID(uint32_t netid);					//permet d'obtenir un client par son baboNet ID
	...
	void		AcceptConnection(uint32_t id);					//va starter une thread pour accepter la nouvelle connection, avec son id unique
	int32_t		UpdateConnections(char *newIP);						//va checker si tel tel connection a terminer, retourne le new babonetID si yen a une quia reussi
