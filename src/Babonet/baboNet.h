...
//fonction prototypes---------------------------------------------------------------------------------------------
	...
	BBNET_DLL_API(int)			bb_peerSend(int32_t peerID,char *data,int typeID,int dataSize,bool safe=false);						//peerID est lidentificateur du peer, un peerID de -1 signifie broadcast packet,safe/unsafe si on tien a ce que le message se rende.
	...
	BBNET_DLL_API(char*)		bb_peerReceive(int32_t *fromPeerID,int *typeID);														//va recevoir un packet de tel tel peer
	BBNET_DLL_API(int)			bb_peerGetIPport(uint32_t babonetID,char *IP,unsigned short *Port);							//permet d'obtenir le IP et le Port d'un Peer
	BBNET_DLL_API(int)			bb_peerDelete(uint32_t baboNetID,bool instant=false);											//permet de retirer un peer dla liste des peers, si instant, le peer n'attendra pas
	...
	BBNET_DLL_API(uint32_t)bb_peerGetBytesSent();																				//retourne le nombre de bytes envoyer par notre peer
	BBNET_DLL_API(uint32_t)bb_peerGetBytesReceived();																			//retourne le nombre de bytes envoyer par notre peer


	//Server functions
	BBNET_DLL_API(int32_t)			bb_serverUpdate(float elapsed,int updateMsg=0,char* NewIP=0);									//donne de l'attention au serveur, retourne -baboNetID dun client disconnecter, retourne baboNetID si un client s'est connecter, retourne 0 on succes, retourn -999999999 on error, le updateMSG : 0=Send/Receive 1=Send 2=Receive
	...
	BBNET_DLL_API(int)			bb_serverSend(char* dataToSend,int dataSize,int typeID,int32_t destination=0,int protocol=0);		//permet d'envoyer des donnees du serveur vers les clients, destination 0 = tout le monde, n = client n, protocol 0 = tcp(safe) 1=Udp(unsafe)
	BBNET_DLL_API(char*)		bb_serverReceive(uint32_t & babonetID,int & typeID, int * size=NULL);							//permet de recevoir le data disponible pour le server
	...
	BBNET_DLL_API(int)			bb_serverDisconnectClient(uint32_t baboNetID);												//permet de deconnecter un client du serveur
	...
	BBNET_DLL_API(uint32_t)bb_serverGetBytesSent();																		//retourne le nombre de bytes envoyer par notre serveur
	BBNET_DLL_API(uint32_t)bb_serverGetBytesReceived();																	//retourne le nombre de bytes envoyer par notre serveur
	BBNET_DLL_API(int)			bb_serverSetClientRate(int nbBytes,uint32_t baboNetID);									//set the max packet size for a send to client

	//Client functions
	BBNET_DLL_API(int)			bb_clientUpdate(uint32_t clientID,float elapsed,int updateMsg=0);							//donne de l'attention au client,retourne 3 lorsque la connection est etablie, retourne 2 si le serveur a disconnecter,retourne 1 si un probleme, 0 on succes, voir le serverUpdate pour le updateMsg : updateMsg est ignorer pendant la connection
	BBNET_DLL_API(uint32_t)bb_clientConnect(const char* HostIP,unsigned short Port);										//permet de connecter notre client a un serveur
	BBNET_DLL_API(int)			bb_clientSend(uint32_t clientID,char* dataToSend,int dataSize,int typeID,int protocol=0);	//permet d'envoyer des donnees du client vers le serveur, protocol 0 = TCP(safe) 1 = UDP(unsafe)
	BBNET_DLL_API(char*)		bb_clientReceive(uint32_t clientID,int *typeID);
	BBNET_DLL_API(char*)		bb_clientGetLastError(uint32_t clientID);													//retourne une version textuel de la derniere erreur, cote client
	BBNET_DLL_API(char*)		bb_clientGetLastMessage(uint32_t clientID);												//retourne une version textuel de ce qui c passer au dernier cycle, cote client
	BBNET_DLL_API(int)			bb_clientDisconnect(uint32_t clientID);													//permet de disconnecter notre client du serveur
	BBNET_DLL_API(uint32_t)bb_clientGetBytesSent(uint32_t clientID);													//retourne le nombre de bytes envoyer par notre client
	BBNET_DLL_API(uint32_t)bb_clientGetBytesReceived(uint32_t clientID);												//retourne le nombre de bytes envoyer par notre client
	BBNET_DLL_API(int)			bb_clientSetRate(uint32_t clientID,int nbBytes);											//set the max packet sizes for send to server
